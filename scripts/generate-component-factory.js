...
function generateComponentFactory() {
  ...
  fs.readdirSync(componentRootPath).forEach((componentFolder) => {
    ...
    if (
      fs.existsSync(path.join(componentFolderFullPath, `${componentFolder}.ts`)) || fs.existsSync(path.join(componentFolderFullPath, `${componentFolder}.tsx`))
    ) {
      const importVarName = componentFolder.replace(/[^\w]+/g, '');

      console.debug(`Registering JSS component ${componentFolder}`);
      imports.push(`import ${importVarName} from '../components/${componentFolder}/${componentFolder}.tsx';`);
